...
class SerialConfigDialog(wx.Dialog):
    """Serial Port confiuration dialog, to be used with pyserial 2.0+
	   ...
	   # SAMPLE OF HOW TO CALL FROM ANOTHER PROGRAM
    ...
    def __init__(self, *args, **kwds):
        ...
        portname = self.serial_ports()
        for portn in portname:
#            portname = serial.device(n)
            n+=1
            self.combo_box_port.Append(portn)
            if self.serial.portstr == portn:
                index = n
        if self.serial.portstr is not None and index is not -1:
...
# for linux ensure user is in dialout group   usermod -a -G dialout fred
# a relogin will be needed by they user before the group addition is recognized
#
#import sys
#import glob
#import serial
